...
CONTAINS
SUBROUTINE SUSDP_DERIV_CTL(KIDIA, KFDIA, PLAIL, PLAIH, PSLT, KLEVS3D, PSSDP2, PSSDP3, YDVEG, YDSOIL, YDAGS, YDCST)
...
REAL(KIND=JPRB),ALLOCATABLE :: ZDL(:)      ! zero-plane displacement height for low veg
REAL(KIND=JPRB),ALLOCATABLE :: ZDH(:)      ! zero-plane displacement height for high veg
REAL(KIND=JPRB),ALLOCATABLE :: ZRHML(:)     ! roughness heat and momentum ratio low veg
REAL(KIND=JPRB),ALLOCATABLE :: ZRHMH(:)     ! roughness heat and momentum ratio high veg
...
IF (YDVEG%LEVZ0) THEN  ! Compute Z0 after Raupach
   NPTTS=KFDIA-KIDIA+1
   IF (.NOT.ALLOCATED(ZDL))            ALLOCATE (ZDL(NPTTS)) 
   IF (.NOT.ALLOCATED(ZDH))            ALLOCATE (ZDH(NPTTS))
   IF (.NOT.ALLOCATED(ZRHML))          ALLOCATE (ZRHML(NPTTS))
   IF (.NOT.ALLOCATED(ZRHMH))          ALLOCATE (ZRHMH(NPTTS))


  ZKAR = -0.4_JPRB ! minus Von Karman cst
  ZHL = 0.5_JPRB   ! Assumed Low vegetation height
  ZHH = 20_JPRB    ! Assumed high vegetation height
 ...
 ! for the moment get the ratio of z0m/z0h from the Lup table or set it to a cst (100)
  IF (RVZ0HL2D(JL) .GT. 0._JPRB )
    ZRHML(JL)=RVZ0ML2D(JL)/RVZ0HL2D(JL)
  ELSE
    ZRHML(JL)=100._JPRB
  ENDIF
  IF (RVZ0HH2D(JL) .GT. 0._JPRB )
    ZRHMH(JL)=RVZ0MH2D(JL)/RVZ0HH2D(JL)
  ELSE
    ZRHMH(JL)=1._JPRB
  ENDIF

  ZDL(JL)  = ZHL* (1._JPRB - ((1._JPRB-EXP(-SQRT(0.5_JPRB*ZCR*PLAIL(JL))))/(-SQRT(0.5_JPRB*ZCR*PLAIL(JL))))) ! zero-plane displacement height 
  ZDH(JL)  = ZHH* (1._JPRB - ((1._JPRB-EXP(-SQRT(0.5_JPRB*ZCR*PLAIH(JL))))/(-SQRT(0.5_JPRB*ZCR*PLAIH(JL))))) ! zero-plane displacement height 
  RVZ0ML2D(JL)  = ZHL* (1._JPRB- ZDL(JL)/ZHL)*EXP(ZKAR*SQRT(2._JPRB/(ZCD*PLAIL(JL))))
  RVZ0MH2D(JL)  = ZHH* (1._JPRB- ZDH(JL)/ZHH)*EXP(ZKAR*SQRT(2._JPRB/(ZCD*PLAIH(JL))))
  RVZ0HH2D(JL)   = RVZ0ML2D(JL)/ZRHML(JL)
  RVZ0HL2D(JL)   = RVZ0MH2D(JL)/ZRHMH(JL)
 ENDDO
...
RRCSOILM3D(:,:) = (1.0_JPRB-RWSATM3D(:,:))*RCGDRYM3D(:,:)+RWCAPM3D(:,:)*RGH2O     
   ...
   IF (ALLOCATED(ZDL)   )     DEALLOCATE (ZDL) 
   IF (ALLOCATED(ZDH)   )     DEALLOCATE (ZDH) 
   IF (ALLOCATED(ZRHML) )     DEALLOCATE (ZRHML)
   IF (ALLOCATED(ZRHMH) )     DEALLOCATE (ZRHMH)
