...
REAL(KIND=JPRB) :: ZCRLamL
REAL(KIND=JPRB) :: ZCRLamH
REAL(KIND=JPRB) :: ZCDLamL
REAL(KIND=JPRB) :: ZCDLamH

REAL(KIND=JPRB) :: ZRVZ0ML2D(KLON)
REAL(KIND=JPRB) :: ZRVZ0MH2D(KLON)
REAL(KIND=JPRB) :: ZRVZ0HL2D(KLON)
REAL(KIND=JPRB) :: ZRVZ0HH2D(KLON)
...
IF (LEVZ0) THEN  ! Compute Z0 after Raupach
  ...
  ZZEPSILON=100._JPRB*EPSILON(ZZEPSILON)
...
ZCRLamL=ZCR*ZLAMeffL
ZCRLamH=ZCR*ZLAMeffH
ZCDLamL=ZCD*ZLAMeffL
ZCDLamH=ZCD*ZLAMeffH


 DO JL=KIDIA,KFDIA
 ! for the moment get the ratio of z0m/z0h from the Lup table or set it to a cst (100/1)
  ...
  IF (PLAIL(JL) > 0.1_JPRB ) THEN
  ZDL(JL)  = 1._JPRB - (1._JPRB-EXP(-SQRT(ZCRLamL*PLAIL(JL))))/(SQRT(ZCRLamL*PLAIL(JL))) ! 0-plane displacement hght
  ZRVZ0ML2D(JL)  = ZHL* (1._JPRB- ZDL(JL))*EXP(ZKAR*SQRT(2._JPRB/(ZCDLamL*PLAIL(JL)))) 
  ELSE
  ZRVZ0ML2D(JL)  = 0.015_JPRB  
  ENDIF
  IF (PLAIH(JL) > 0.15_JPRB ) THEN  
  ZDH(JL)  = 1._JPRB - (1._JPRB-EXP(-SQRT(ZCRLamH*PLAIH(JL))))/(SQRT(ZCRLamH*PLAIH(JL))) ! 0-plane displacement hght 
  ZRVZ0MH2D(JL)  = ZHH* (1._JPRB- ZDH(JL))*EXP(ZKAR*SQRT(2._JPRB/(ZCDLamH*PLAIH(JL))))
  ELSE
  ZRVZ0MH2D(JL)  = 0.5_JPRB
  ENDIF 
 
  ZRVZ0HL2D(JL)   = ZRVZ0ML2D(JL)/ZRHML(JL)
  ZRVZ0HH2D(JL)   = ZRVZ0MH2D(JL)/ZRHMH(JL)
 ENDDO
!print*, "After LEVZ0 "

ELSE
 DO JL=KIDIA,KFDIA
  ZRVZ0ML2D(JL)=RVZ0ML2D(JL)
  ZRVZ0MH2D(JL)=RVZ0MH2D(JL)
  ZRVZ0HL2D(JL)=RVZ0HL2D(JL)
  ZRVZ0HH2D(JL)=RVZ0HH2D(JL)
 ENDDO
ENDIF
...
!       - Sea ice
    ...
    ENDIF
!       - Wet skin
    PZ0MTI(JL,3)=PCVL(JL)*ZRVZ0ML2D(JL)+PCVH(JL)*ZRVZ0MH2D(JL)&
      & +(1.-PCVL(JL)-PCVH(JL))*RVZ0M_BARE
!       - Low Vegetation
    PZ0MTI(JL,4)=ZRVZ0ML2D(JL)
!       - Exposed snow
! blend between roughness of low vegetation and soil and roughness of ice caps, when snow depth below 25cm 
    ZMLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*ZRVZ0ML2D(JL)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0M_BARE
...
!       - High vegetation
    PZ0MTI(JL,6)=ZRVZ0MH2D(JL)
!       - Sheltered snow
    PZ0MTI(JL,7)=ZRVZ0MH2D(JL)
!       - Bare soil
...
!       - URBAN
    IF (LEURBAN) THEN
      ...
      PZ0MTI(JL,3)=(ZPCVL*ZRVZ0ML2D(JL)+ZPCVH*ZRVZ0MH2D(JL)&
      & +ZPCVB*RVZ0M_BARE+RURBZTM*ZURBF)
      ZMLOW=ZPCVL/(1.0_JPRB-ZPCVH)*ZRVZ0ML2D(JL)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0M_BARE
...
!   - Wet skin
DO JL=KIDIA,KFDIA
  PZ0MTI(JL,JTILE)=PCVL(JL)*ZRVZ0ML2D(JL)+PCVH(JL)*ZRVZ0MH2D(JL)+(1.-PCVL(JL)-PCVH(JL))*RVZ0M_BARE
  PZ0HTI(JL,JTILE)=PCVL(JL)*ZRVZ0HL2D(JL)+PCVH(JL)*ZRVZ0HH2D(JL)+(1.-PCVL(JL)-PCVH(JL))*RVZ0H_BARE
...
!   - Low Vegetation
DO JL=KIDIA,KFDIA
  PZ0MTI(JL,JTILE)=ZRVZ0ML2D(JL)
  PZ0HTI(JL,JTILE)=ZRVZ0HL2D(JL)
  PZ0QTI(JL,JTILE)=PZ0HTI(JL,JTILE)
ENDDO
...
DO JL=KIDIA,KFDIA
  ZMLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*ZRVZ0ML2D(JL)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0M_BARE
  ZHLOW=PCVL(JL)/(1.0_JPRB-PCVH(JL))*ZRVZ0HL2D(JL)+(1.0_JPRB-PCVL(JL)-PCVH(JL))/(1.0_JPRB-PCVH(JL))*RVZ0H_BARE
  ...
  ! The actual z0h is a log-average of vegetation and snow, dependending on ZSNWGHT
    ...
    IF (LEURBAN) THEN
      ...
      IF (ZURBF.GT.0.0_JPRB) THEN
        ZMLOW=ZPCVL/(1.0_JPRB-ZPCVH)*ZRVZ0ML2D(JL)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0M_BARE
        ZHLOW=ZPCVL/(1.0_JPRB-ZPCVH)*ZRVZ0HL2D(JL)+ZPCVB/(1.0_JPRB-ZPCVH)*RVZ0H_BARE
...
!   - High vegetation
DO JL=KIDIA,KFDIA
  PZ0MTI(JL,JTILE)=ZRVZ0MH2D(JL)
  PZ0HTI(JL,JTILE)=ZRVZ0HH2D(JL)
  PZ0QTI(JL,JTILE)=PZ0HTI(JL,JTILE)
ENDDO
