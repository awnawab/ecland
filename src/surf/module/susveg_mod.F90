MODULE SUSVEG_MOD
CONTAINS
SUBROUTINE SUSVEG(LD_LELAIV,LD_LEVVCOV,LD_LEVZ0,LD_LECTESSEL,LD_LEAGS,LD_LEFARQUHAR,&
...
LOGICAL,         INTENT(IN)    :: LD_LEVVCOV
LOGICAL,         INTENT(IN)    :: LD_LEVZ0
...
ASSOCIATE(RETV=>YDCST%RETV, &
 ...
 & LEFARQUHAR=>YDVEG%LEFARQUHAR, LELAIV=>YDVEG%LELAIV, LEVVCOV=>YDVEG%LEVVCOV, &
 & LEVZ0=>YDVEG%LEVZ0, NVTILES=>YDVEG%NVTILES, NVTYPES=>YDVEG%NVTYPES, &
...
IF(.NOT.ALLOCATED(YDVEG%RVCOV)) ALLOCATE(YDVEG%RVCOV(0:IVTYPES))
! IF (.NOT.LEVVCOV) ! standard RVCOV use
YDVEG%RVCOV(1)=0.9_JPRB     ! Crops, Mixed Farming
YDVEG%RVCOV(2)=0.85_JPRB    ! Short Grass
YDVEG%RVCOV(3)=0.9_JPRB     ! Evergreen Needleleaf Trees
YDVEG%RVCOV(4)=0.9_JPRB     ! Deciduous Needleleaf Trees
...
YDVEG%RVCOV(20)=0.6_JPRB    ! Water and Land Mixtures
YDVEG%RVCOV(0)=YDVEG%RVCOV(8)
!ELSE ! use RVCOV to proxy k*Omg of Lambert-Beer based variability with LAI
!YDVEG%RVCOV(1)=-0.458_JPRB     ! Crops, Mixed Farming
!YDVEG%RVCOV(2)=-0.456_JPRB    ! Short Grass
!YDVEG%RVCOV(3)=-0.351_JPRB     ! Evergreen Needleleaf Trees
!YDVEG%RVCOV(4)=-0.381_JPRB     ! Deciduous Needleleaf Trees
!YDVEG%RVCOV(5)=-0.396_JPRB     ! Deciduous Broadleaf Trees
!YDVEG%RVCOV(6)=-0.390_JPRB    ! Evergreen Broadleaf Trees
!YDVEG%RVCOV(7)=-0.5_JPRB     ! Tall Grass/mixed crops
!YDVEG%RVCOV(8)=-0.5_JPRB       ! Desert
!YDVEG%RVCOV(9)=-0.375_JPRB       ! Tundra
!YDVEG%RVCOV(10)=-0.5_JPRB    ! Irrigated Crops
!YDVEG%RVCOV(11)=-0.5_JPRB    ! Semidesert/sparse vegetation
!YDVEG%RVCOV(12)=-0.5_JPRB      ! Ice Caps and Glaciers
!YDVEG%RVCOV(13)=-0.419_JPRB    ! Bogs and Marshes
!YDVEG%RVCOV(14)=-0.5_JPRB      ! Inland Water
!YDVEG%RVCOV(15)=-0.5_JPRB      ! Ocean
!YDVEG%RVCOV(16)=-0.438_JPRB      ! Evergreen Shrubs
!!YDVEG%RVCOV(17)=-0.3_JPRB      ! Deciduous Shrubs
!YDVEG%RVCOV(17)=-0.448_JPRB      ! Deciduous Shrubs (testing 0.4)
!YDVEG%RVCOV(18)=-0.5_JPRB    ! Mixed Forest/woodland/Broad Savanah
!YDVEG%RVCOV(19)=-0.5_JPRB    ! Interrupted Forest
!YDVEG%RVCOV(20)=-0.5_JPRB    ! Water and Land Mixtures
!YDVEG%RVCOV(0)=YDVEG%RVCOV(8)
!ENDIF
