...
ASSOCIATE(LEFLAKE=>YDFLAKE%LEFLAKE,LEURBAN=>YDURB%LEURBAN, &
 ...
 & LECTESSEL=>YDVEG%LECTESSEL, LELAIV=>YDVEG%LELAIV, RLAIINT=>YDVEG%RLAIINT, LEVVCOV=>YDVEG%LEVVCOV, &
...
DO JL=KIDIA,KFDIA
 ...
 ELSE ! Lambert-Beer based variability with LAI
    ! Option1
!    PCVL(JL)=PCVLC(JL)*(1.0_JPRB-EXP(-0.6_JPRB*PLAIL(JL)))
!    PCVH(JL)=PCVHC(JL)*(1.0_JPRB-EXP(-0.6_JPRB*PLAIH(JL)))

    ! Option2: k=0.5 OmegaLow=0.8 , OmegaHigh=0.6
!    PCVL(JL)=PCVLC(JL)*(1.0_JPRB-EXP(-0.4_JPRB*PLAIL(JL)))
!    PCVH(JL)=PCVHC(JL)*(1.0_JPRB-EXP(-0.3_JPRB*PLAIH(JL)))

    ! Option3: k.Omg=0.9 Low= , k.Omg=0.65 High
    PCVL(JL)=PCVLC(JL)*(1.0_JPRB-EXP(-0.90_JPRB*PLAIL(JL)))
    PCVH(JL)=PCVHC(JL)*(1.0_JPRB-EXP(-0.65_JPRB*PLAIH(JL)))

    ! Option4:use RVCOV to proxy k.Omg of Lambert-Beer (***Also could be optimised***)
!    PCVL(JL)=PCVLC(JL)*(1.0_JPRB-EXP(RVCOVL2D(JL)*PLAIL(JL)))
!    PCVH(JL)=PCVHC(JL)*(1.0_JPRB-EXP(RVCOVH2D(JL)*PLAIH(JL)))

!    PCVH(JL)=PCVHC(JL)*(1.0_JPRB-EXP(-0.8_JPRB*(PLAIH(JL)+0.8)))
!   PCVL(JL)=PCVLC(JL)*(1.0_JPRB-EXP(-0.9_JPRB*(PLAIL(JL)+0.5)))
