...
! SUM OVER FIRST 8 TILES
  ZSPECTRALEMISS(JL,:) = SUM(ZEMISSTI(JL,:,1:8) * SPREAD(PFRTI(JL,1:8),1,NLWEMISS), 2)

! URBAN
  IF (LEURBAN) THEN
    ZEMISSTI(JL,:,10) = RURBEMIS
    ZSPECTRALEMISS(JL,:) = SUM(ZEMISSTI(JL,:,1:8) * SPREAD(PFRTI(JL,1:8),1,NLWEMISS), 2) +&
    & ZEMISSTI(JL,:,10) * SPREAD(PFRTI(JL,10),1,NLWEMISS)
  ENDIF

  IF (LEFLAKE) THEN
    ZSPECTRALEMISS(JL,:) = ZSPECTRALEMISS(JL,:) + PFRTI(JL,9)*ZEMISSTI(JL,:,9)
  ENDIF
