...
REAL(KIND=JPRB)   , INTENT(INOUT)  :: PRUSN(:)
REAL(KIND=JPRB)   , INTENT(INOUT)  :: PMELTSN(:,:)
REAL(KIND=JPRB)   , INTENT(INOUT)  :: PFREZSN(:,:)

REAL(KIND=JPRB)   , INTENT(INOUT)  :: PDHTSS(:,:,:)
REAL(KIND=JPRB)   , INTENT(INOUT)  :: PDHSSS(:,:,:)
...
REAL(KIND=JPRB) :: ZGAM(KLON,KLEVSN)
...
! Add radiative flux to second to bottom layer:
    ...
    ELSE
      ! MULTI LAYER
      CALL TRISOLVER(KLON,KLACT,KLEVSN,ZTA(:,:),ZTB(:,:),ZTC(:,:),ZTR(:,:),ZTSTAR(:,:),ZGAM(:,:),JLL=JL)
