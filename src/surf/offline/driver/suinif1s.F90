...
USE YOMGPD1S , ONLY : GPD_SDP2, GPD_SDP3, VFTVL, VFTVH, VFSOTY, VFLAIL, VFLAIH
...
DO IST=1,NPOI,NPROMA
  ...
  IST1 = 1
  IEND1 = IEND-IST+1
  CALL SUSDP_DERIV(YDSURF=YDSURF,KIDIA=IST1,KFDIA=IEND1,PLAIL=VFLAIL,PLAIH=VFLAIH,PSLT=VFSOTY(IST:IEND),&
          & PSSDP2=GPD_SDP2(IST:IEND,:),PSSDP3=GPD_SDP3(IST:IEND,:,:),KLEVS3D=NCSS)
ENDDO
!$OMP END PARALLEL DO
